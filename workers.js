export default {
  async fetch(request, env) {
    ...
    // Insert visit record
    const visitTime = currentUTCPlus8Date.toISOString();
    ...
    const todayHits = (await db.prepare(`SELECT COUNT(*) as count FROM visits WHERE tag_id = ? AND visit_time >= ?`).bind(tagId, currentUTCPlus8Date.toISOString().slice(0, 10)).first())?.count || 0;
